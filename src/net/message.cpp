...
void writeU16Be(std::uint16_t value, std::uint8_t* out)
{
    out[0] = static_cast<std::uint8_t>((value >> 8) & 0xff);
    out[1] = static_cast<std::uint8_t>(value & 0xff);
}

void writeU32Be(std::uint32_t value, std::uint8_t* out)
{
...
std::optional<Message> FrameCodec::tryDecode(ByteBuffer& buffer, std::size_t max_payload_size)
{
    if (buffer.size() < header_size) {
        return std::nullopt;
    }

    const auto header = decodeHeader(buffer.data(), max_payload_size);
...
FrameHeader FrameCodec::decodeHeader(const std::uint8_t* data, std::size_t max_payload_size)
{
    const auto payload_size = readU32Be(data);
    ...
    return FrameHeader{
        payload_size,
        static_cast<MessageType>(readU16Be(data + 4)),
        readU16Be(data + 6),
    };
}

void FrameCodec::encodeHeader(const FrameHeader& header, std::uint8_t* out)
{
    writeU32Be(header.payload_size, out);
    writeU16Be(static_cast<std::uint16_t>(header.type), out + 4);
    writeU16Be(header.flags, out + 6);
}
